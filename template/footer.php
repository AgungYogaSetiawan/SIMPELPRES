        ...
        <script type="text/javascript">
            function submitForm(form) {
                ...
                .then((isOkay) => {
                    if (isOkay) {
                        form.submit();
                        window.location.href = 'formulir2a.php';
                    }
                });
                return false;
            }
    ...
    <script>
        function hitungUmur() {
            ...
            var tanggalLahir = document.getElementById("tgl_lahir_anak").value;

            // memecah string tanggal lahir menjadi tahun, bulan, dan tanggal
            var parts = tanggalLahir.split("-");

            // membuat objek Date dari tanggal lahir
            var tanggalLahirObject = new Date(parts[0], parts[1] - 1, parts[2]);

            // menghitung selisih tanggal lahir dan tanggal sekarang dalam milisecond
            var selisih = Date.now() - tanggalLahirObject.getTime();

            // menghitung umur dalam tahun
            var hari = Math.floor(selisih / (1000 * 60 * 60 * 24));

            // menghitung selisih bulan
            var umur = Math.floor(hari / 30);

            // menampilkan umur ke dalam element HTML
            document.getElementById("umur").value = umur;
        };

        function hitungUsia2C() {
            // tanggal lahir dalam format YYYY-MM-DD
            ...
            // menghitung selisih bulan
            var umur = Math.floor(hari / 30);
            var umur = Math.floor(umur / 12);

            // menampilkan umur ke dalam element HTML
            document.getElementById("usia_anak").value = umur;
        };

        function hitungPersen() {
            var jml_diterima = document.getElementById("jml_diterima_lengkap").value;
            var jml_seharusnya = parseInt(6);
            var result = Math.floor(parseInt(jml_diterima) / jml_seharusnya * 100);
            if (!isNaN(result)) {
                document.getElementById('persen').value = result.toFixed();
            }
        }

        function hitungPersen2() {
            var jml_diterima = document.getElementById("jml_diterima_lengkap").value;
            var jml_seharusnya = parseInt(10);
            var result = Math.floor(parseInt(jml_diterima) / jml_seharusnya * 100);
            if (!isNaN(result)) {
                document.getElementById('persen').value = result.toFixed();
            }
