<?php
    ...
    $id_formulir_duaB = $_GET['id2'];
    $sql = "SELECT r.*, f.*, b.*, g.status_gizi, h.hasil, p.deskripsi, e.* FROM tb_intervensi r, tb_formulir2b f, tb_baduta b, tb_gizi_anak g, tb_hasil h, tb_pencegahan p, tb_rekomendasi e WHERE r.id_formulir_duaB=f.id_formulir_duaB AND f.id_baduta=b.id_baduta AND b.id_gizi_anak=g.id_gizi_anak AND b.id_hasil=h.id_hasil AND e.id_pencegahan = p.id_pencegahan AND r.id_intervensi = e.id_intervensi AND g.status_gizi = 'STUNTING' AND r.id_intervensi = '$id_rekomendasi'";
    $result = mysqli_query($konek,$sql);
    $data = mysqli_fetch_array($result);

    // Ambil data terpilih sebelumnya dari tabel 'tb_data_terpilih' berdasarkan ID
    $query_terpilih = mysqli_query($konek, "SELECT id_pencegahan FROM tb_rekomendasi WHERE id_intervensi = $id_rekomendasi");
    while ($row_terpilih = mysqli_fetch_assoc($query_terpilih)) {
        $data_terpilih[] = $row_terpilih['id_pencegahan'];
    }


    if(isset($_POST['edit'])){
        $id_intervensi = $_POST['id_intervensi'];
        $id_pencegahan = $_POST['id_pencegahan'];

        $fetchquery = mysqli_query($konek, "SELECT * FROM tb_rekomendasi WHERE id_intervensi='$id_intervensi'");

        $arrPencegahan = [];
        foreach($fetchquery as $fetchrow){
            $arrPencegahan[] = $fetchrow['id_pencegahan'];
        }

        // insert data
        foreach($id_pencegahan as $pencegahan) {
            if(!in_array($pencegahan, $arrPencegahan)) {
                $insert_query = mysqli_query($konek, "INSERT INTO tb_rekomendasi (id_intervensi,id_pencegahan) VALUES ('$id_intervensi','$pencegahan')");
            }
        }

        // delete data
        foreach($arrPencegahan as $fetchrow) {
            if(!in_array($fetchrow, $id_pencegahan)) {
                $delete_query = mysqli_query($konek, "DELETE FROM tb_rekomendasi WHERE id_intervensi='$id_intervensi' AND id_pencegahan='$fetchrow'");
            }
        }
...
<head>
    ...
    <title>Laporan Rekomendasi Intervensi - Edit Data</title>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card mb-4 mt-2">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold">Edit Data Rekomendasi Intervensi</h6>
                        </div>
                        <div class="card-body">
                            <form action="" method="POST">
                                    <input type="hidden" id="id_intervensi" name="id_intervensi" value="<?php echo $data['id_intervensi']; ?>" readonly>
                                    <div class="row">
                                            <div class="col">
                                                <!-- kelurahan -->
                                        ...
                                        <div class="row">
                                            <div class="col">
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-10 mt-2">
                                                        <select class="form-select select2" name="id_gizi_anak" id="id_gizi_anak" style="width: 250px;" disabled>
                                                        <?php 
                                                        include '../../setting/koneksi.php';
                                        ...
                                        <div class="row">
                                            <div class="col">
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-10">
                                                        <select class="form-select select2" name="id_hasil" id="id_hasil" style="width: 250px;" disabled>
                                                        <?php 
                                                        include '../../setting/koneksi.php';
                                                        $result = mysqli_query($konek, "SELECT * FROM tb_hasil");
                                                        while($row = mysqli_fetch_array($result)){?> 
                                                        <option value="<?php echo $row['id_hasil']?>" <?php if($data["id_hasil"] == $row['id_hasil']){echo "SELECTED";} ?>><?php echo $row['hasil']?></option>
                                                        <?php } ?>
                                                        </select>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <select class="form-select select2" name="id_pencegahan[]" id="id_pencegahan" style="width: 250px;" multiple>
                                                    ...
                                                    while ($data = mysqli_fetch_array($query)) {
                                                        // Periksa apakah data saat ini sudah dipilih sebelumnya
                                                        $selected = in_array($data['id_pencegahan'], $data_terpilih) ? 'selected' : '';
                                                        ?>
                                                        <option value="<?= $data['id_pencegahan']; ?>" <?= $selected; ?>><?php echo $data['pencegahan']; ?></option>
                                                    <?php
                                                    }
                                                    ?>
                                                </select>
